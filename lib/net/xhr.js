...
function send(url, options) {
    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState === state.HEADERS_RECEIVED) {
            onHeaders(xhr, xhr.statusText, parseHeader(xhr.getAllResponseHeaders()));
        } else if (xhr.readyState === state.LOADING) {
            if (xhr.responseText) {
                onChunk(xhr, xhr.responseText);
            }
            ...
            if (xhr.status < 200 || xhr.status > 299) {
                errorFn(xhr, xhr.statusText);
            } else {
                onChunk(xhr, xhr.responseText, true);
            }
            // Call complete at the end
            completeFn(xhr, xhr.statusText);
        }
    };
